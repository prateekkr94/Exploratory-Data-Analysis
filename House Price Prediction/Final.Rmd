...
```{r}
#ggplot(all[!is.na(all$SalePrice),], aes(x=as.factor(LotConfig), y=SalePrice)) +y
#        geom_bar(stat='summary', fun.y = "median", fill='blue') +
#        scale_y_continuous(breaks= seq(0, 800000, by=100000), labels = comma) +
#        geom_label(stat = "count", aes(label = ..count.., y = ..count..))
```
...
```{r}
nzvar <-nearZeroVar(DFdummies)
DFdummies <-DFdummies[-nzvar]
colnames(DFdummies)
```
...
```{r}
combined$Id <- NULL
train_new <- combined[1:1460, ]
test_new <- combined[1461:2917, ]
train_sp <- all[1:1460, ]
train_sp <- train_sp$SalePrice
test_sp <- all[1461:2917, ]
test_sp <- test_sp$SalePrice
```
...
#lm model
```{r}
lmFit1 <- train(x = train_new, y = train_sp, method = "lm", trControl = ctrl)
lmFit1

predicted <- predict(lmFit1, train_new) 
#plot(train_sp, predicted, main = "Linear Regression Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(predicted, train_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=train_sp, y=predicted))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
...
ridgeRegFit <- train(x = train_new, y = train_sp, method = "ridge", 
...
predicted <- predict(ridgeRegFit, train_new) 
...
xyplot(resid(ridgeRegFit) ~ predict(ridgeRegFit), type = c("p", "g"), 
       xlab = "Predicted", ylab = "Residuals")
```
...
lasso_mod <- train(x=train_new, y=train_sp, method='glmnet', trControl= ctrl, preProc = c("center", "scale"), tuneGrid=lassoGrid) 
lasso_mod
plot(lasso_mod)

ggplot(lasso_mod)

predicted <- predict(lasso_mod, train_new) 
...
xyplot(resid(lasso_mod) ~ predict(lasso_mod), type = c("p", "g"), 
       xlab = "Predicted", ylab = "Residuals")
```
...
enetTune <- train(x=train_new, y=train_sp, method = "enet", 
...
predicted <- predict(enetTune, train_new) 
#plot(train_sp, predicted, main = "Elastic Net Observed vs Predicted", xlab = "Observed", ylab = "Predicted")
...
ggplot(data=result, aes(x=train_sp, y=predicted))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("Elastic Net Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")

xyplot(resid(enetTune) ~ predict(enetTune), type = c("p", "g"), 
       xlab = "Predicted", ylab = "Residuals")
```

#knn
```{r}
knnModel <- train(x=train_new, y=train_sp, method = "knn", preProc = c("center", "scale"), trControl = ctrl,
                  tuneLength = 20)
knnModel
plot(knnModel)
...
knnPred <- predict(knnModel, newdata = train_new)

#plot(train_sp, knnPred, main = "KNN Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(knnPred, train_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=train_sp, y=knnPred))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("KNN Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
```
...
marsModel = train(x=train_new, y=train_sp, method="earth", preProc=c("center", "scale"), 
                  tuneGrid=marsGrid,trControl = ctrl)

marsModel
#summary(marsModel) # Selects X1-X5
plot(marsModel)
ggplot(marsModel)
marsPred <- predict(marsModel, newdata=train_new)

#plot(train_sp, marsPred, main = "MARS Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(marsPred, train_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=train_sp, y=marsPred))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("MARS Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
```

#svm
```{r}
svmModel = train(x=train_new, y=train_sp, method="svmRadial", preProc=c("center", "scale"), tuneLength=20, trControl = ctrl)
...
ggplot(svmModel)+ scale_x_continuous(trans='log2')


svmPred = predict(svmModel, newdata=train_new)

plot(train_sp, svmPred, main = "SVM Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(svmPred, train_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=train_sp, y=svmPred))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("SVM Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
```

#nn
```{r}
nnGrid = expand.grid(.size=1:10, .decay=c(0,0.01,0.1))

nnetModel = train(x=train_new, y=train_sp, method="nnet", preProc=c("center", "scale"),
                  linout=TRUE,trace=FALSE,MaxNWts=10 * (ncol(train_new)+1) + 10 + 1, maxit=500, trControl = ctrl, tuneGrid = nnGrid)
...
nnetPred = predict(nnetModel, newdata=train_new)

plot(train_sp, predicted, main = "Neural Net Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(nnetPred, train_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=train_sp, y=nnetPred))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("Neural Net Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
```


Ridge and Lasso are the best model, testing on testdata
...
```{r}
predicted_ridge <- predict(ridgeRegFit, test_new) 
plot(test_sp, predicted_ridge, main = "Ridge Regression Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(predicted_ridge, train_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=train_sp, y=predicted_ridge))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("Ridge Regression Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
...
postResample(pred=predicted_ridge, obs=test_sp)
#varImp(ridgeRegFit)$importance

x <- varImp(ridgeRegFit, scale = F)
plot(x, top = 15)
```
...
#lasso
```{r}
predicted_lasso <- predict(lasso_mod, test_new) 
plot(test_sp, predicted_lasso, main = "Lasso Regression Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(predicted_lasso, test_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=test_sp, y=predicted_lasso))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("Lasso Regression Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
...
postResample(pred=predicted_lasso, obs=test_sp)
#varImp(lasso_mod)$importance

x <- varImp(lasso_mod, scale = F)
plot(x, top = 15)
```
```{r}
sub_avg <- (predicted_ridge+3*predicted_lasso)/4
postResample(pred=sub_avg, obs=test_sp)
```

#enet
```{r}
predicted_enet <- predict(enetTune, test_new) 
#plot(train_sp, predicted, main = "Elastic Net Observed vs Predicted", xlab = "Observed", ylab = "Predicted")

result = cbind(predicted_enet, test_sp)
result = as.data.frame(result)

ggplot(data=result, aes(x=test_sp, y=predicted_enet))+
        geom_point(col='blue') + geom_smooth(method = "lm", se=FALSE, color="black", aes(group=1)) +
        scale_y_continuous(breaks= seq(10, 14, by=0.5), labels = comma) + scale_x_continuous(breaks= seq(10, 14, by=0.5), labels = comma)+
  ggtitle("Elastic Net Observed vs Predicted") +
  xlab("Observed") + ylab("Predicted")
